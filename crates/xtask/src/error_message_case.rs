...
const CONTEXT_METHODS: &[&str] = &["context", "with_context", "wrap_err", "wrap_err_with", "ok_or_eyre"];
...
pub fn check(fix: bool) -> eyre::Result<CheckOutcome> {
    ...
    // Skip descending into `target/` and `.git` rather than walking them and
    // filtering after the fact -- `target/` alone can be tens of gigabytes.
    let walk = WalkDir::new(&repo_root)
        .into_iter()
